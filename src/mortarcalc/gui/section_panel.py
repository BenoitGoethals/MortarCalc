...
class SectionPanel(QWidget):
    ...
    def _build_pieces_group(self) -> QGroupBox:
        ...
        btns.addWidget(b_add); btns.addWidget(b_del); btns.addStretch(1); btns.addWidget(b_reset)
    ...
    def _build_groups_panel(self) -> QGroupBox:
        box = QGroupBox("Fire Sections — click a section to edit")
        wrap = QVBoxLayout(box)
        ...
        btn_row.addWidget(b_new); btn_row.addStretch(1); btn_row.addWidget(b_reset_sec)
        wrap.addLayout(btn_row)

        # Compact section list — single row of clickable buttons-as-items
        self.groups_list = QListWidget()
        self.groups_list.setFlow(QListWidget.LeftToRight)
        self.groups_list.setWrapping(True)
        self.groups_list.setResizeMode(QListWidget.Adjust)
        self.groups_list.setSpacing(4)
        self.groups_list.setFixedHeight(46)
        self.groups_list.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        self.groups_list.setSelectionMode(QAbstractItemView.NoSelection)
        # Single click opens the modal; no inline detail.
        self.groups_list.itemClicked.connect(self._open_group_dialog)
    ...
    def _refresh_groups_list(self) -> None:
        ...
        for g in self.peloton.groups:
            ...
            item.setToolTip("Click to edit section")
